...
class _MapaLluviaScreenState extends State<MapaLluviaScreen> {
  ...
  String getMapboxStyleUrl() {
    ...
    final style = isNight ? 'mapbox/navigation-night-v1' : 'mapbox/navigation-day-v1';
  ...
  Future<void> fetchRainViewerFrames() async {
    ...
    if (response.statusCode == 200) {
      ...
      final timestamps = pastFrames.map<int>((frame) => frame['time'] as int).toList();
  ...
  void previousFrame() {
    _animationTimer?.cancel();
    setState(() {
      _currentFrameIndex = (_currentFrameIndex - 1 + _rainFrames.length) % _rainFrames.length;
      _isPlaying = false;
    });
  }
  ...
  String getFormattedTime(int unixTimestamp) {
    final date = DateTime.fromMillisecondsSinceEpoch(unixTimestamp * 1000).toLocal();
  ...
  Widget build(BuildContext context) {
    final radarTile = _rainFrames.isNotEmpty
        ? TileLayer(
                      ...
                      children: [
                        ...
                        Text(
                          _rainFrames.isNotEmpty
                              ? getFormattedTime(_rainFrames[_currentFrameIndex])
                              : '',
